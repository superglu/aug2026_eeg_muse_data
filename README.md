...
```sh
python ingestion/src/stream_eeg.py
```

**Terminal 2** — subscribe to the stream with any (or several) of:

```sh
python ingestion/src/print_eeg.py                  # raw electrode values + band powers, once per second
python ingestion/src/plot_live.py                  # live scrolling plot of all four channels
python ingestion/src/record_eeg.py --user gary     # record 60 s to data/gary_<timestamp>.csv (--duration to change, 0 = until Ctrl+C)
```

Analyze a recording offline:

```sh
python ingestion/src/analyze_session.py data/eeg_<timestamp>.csv
```
...
2. Record one minute: `python ingestion/src/record_eeg.py --user <name>`.
...
- To consume the stream in your own code, resolve it with `pylsl` — see the top of `ingestion/src/print_eeg.py`.
...
```sh
python ingestion/muse_to_fif.py data/<recording>.csv       # convert a Muse recording to fif_in/<name>_raw.fif
python ingestion/clean_eeg.py                              # clean every .fif in fif_in/
python ingestion/clean_eeg.py --repair-channels Cz T3      # also fully reconstruct named channels
python ingestion/clean_eeg.py --target-channels 64         # upsample the montage to 64 channels
python ingestion/clean_eeg.py --bad-segments 5:6 10:11:C3  # reconstruct time spans (start:end[:channel])
```
