...
class CheckpointManager:
    def __init__(self, args: CheckpointArgs):
        ...
        self.load_optimizer_state = args.load_optimizer_state
    ...
    def get_state_dict(
        ...
        # FROM LOAD_FROM_CHECKPOINT FUNCTION
        state_dict = {}
        if optimizer is not None:
    ...
    def load(
        ...
        if False:
            print(f" INSIDE CHECKPOINT MANAGER LOAD FUNCTION BEFORE DCP LOAD")
            print(f" ***** state_dict keys: {state_dict.keys()}")
            # print(f" ***** state_dict optim keys: {state_dict["optim"].keys()}")
            # for key in state_dict["optim"].keys():
            #     print(f"***** key: {key}")
            #     print(f"***** value: {state_dict["optim"][key]}")
            # die

        dcp.load(state_dict, checkpoint_id=path, planner=dcp.default_planner.DefaultLoadPlanner(allow_partial_load=True))
        logger.info("State dict loaded.")
        logger.info("Reloading model and optim")


        if False:
            print(f" INSIDE CHECKPOINT MANAGER LOAD FUNCTION AFTER DCP LOAD")
            print(f" ***** state_dict keys: {state_dict.keys()}")
            # print(f" ***** state_dict optim keys: {state_dict["optim"].keys()}")
            # for key in state_dict["optim"].keys():
            #     print(f"***** key: {key}")
        ...
        if not self.load_optimizer_state:
            logger.warning("Resetting optimizer state and scheduler's last_epoch (load_optimizer_state=False)")
            # Get fresh optimizer state dict (initialized state)
            state_dict["optim"] = optimizer.state_dict()
            # Reset scheduler's last_epoch to the current step so it uses the new schedule
        ...
        if True: #was - if self.load_optimizer_state:
            ...
            # Only set model state, skip optimizer
            set_model_state_dict(model, model_state_dict=state_dict["model"])
            logger.info("Model reloaded (optimizer state skipped - using fresh state)")
